...
export default function CubeInput() {
  ...
  // --- Estados para el envío al Robot ---
  const [robotFinished, setRobotFinished] = useState(false);
  const [robotTime, setRobotTime] = useState('');
  ...
  const handleSendToRobot = async () => {
    ...
    try {
      ...
      if (resp.ok && (data.exito || data.estado === "real" || data.estado === "simulacion")) {
        // ¡El robot terminó!
        setRobotFinished(true);
        // Si el backend envía el tiempo en 'data.tiempo', lo guardamos.
        // Si viene en otro formato según tu backend, ajústalo aquí.
        setRobotTime(data.tiempo || '0.00');
      ...
      {cube[faceKey].map((color, index) => {
        ...
        return (
          <div
            ...
            className={`w-10 h-10 border border-gray-600 rounded-sm cursor-pointer transition-transform hover:scale-105 ${index === 4 ? 'ring-2 ring-gray-200' : ''
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center py-10 relative">
          ...
          <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-700 w-full max-w-xl">
            ...
            <div className="grid grid-cols-3 gap-2 bg-gray-900 p-3 rounded-xl border border-gray-700 mx-auto w-fit mb-6 shadow-inner">
              {modalFaceState.map((colorValue, index) => (
                <div
                  key={`modal-${index}`}
                  className={`w-28 h-28 border-[3px] border-black rounded-sm overflow-hidden relative bg-gray-800 ${index === 4 ? 'ring-2 ring-white z-10 scale-105' : ''
                    }`}
                >
                  {index === 4 ? (
                      ...
                      {PALETTE_ORDER.map((paletaColor) => {
                        ...
                        return (
                          <div
                            ...
                            className={`cursor-pointer transition-all border border-black/20 flex justify-center items-center ${isSelected
      ...
      <div className="flex flex-col md:flex-row gap-12 items-start relative z-10">
        ...
        <div className="bg-gray-800 p-6 rounded-xl flex flex-col gap-4 w-64 border border-gray-700">
          ...
          <button onClick={handleSendToRobot} disabled={!canSendToRobot || sendingRobot} className={`font-bold py-2 rounded-lg transition-all text-sm shadow-lg ${canSendToRobot && !sendingRobot ? 'bg-blue-600 hover:bg-blue-500 text-white cursor-pointer opacity-100' : 'bg-gray-700 text-gray-500 cursor-not-allowed opacity-50'}`}>
            {sendingRobot ? '⏳ Robot trabajando...' : 'Enviar al Robot 🤖'}
          </button>
        </div>
      </div>
      ...
      {/* Modal de Finalización del Robot */}
      {showRobotModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 transition-opacity">
          <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-700 w-full max-w-sm text-center">
            <h3 className="text-3xl font-bold mb-2 text-emerald-400">🎉 ¡Listo!</h3>
            <p className="text-gray-300 mb-6">El brazo robótico ha completado la resolución del cubo.</p>

            <div className="bg-gray-900 p-4 rounded-xl border border-gray-700 mb-6">
              <span className="text-sm text-gray-400 block mb-1">Tiempo oficial:</span>
              <span className="text-4xl font-mono text-white font-bold">{robotTime}s</span>
            </div>
            <button
              onClick={() => setShowRobotModal(false)}
              className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-lg transition-colors shadow-lg w-full"
            >
              Cerrar
            </button>
          </div>
